...
def test_max_conformers(max_conformers):
    ...
    assert (
        len(ConformerGenerator.generate(molecule, ConformerSettings(max_conformers=max_conformers))) == max_conformers
    )


@pytest.mark.skip(
    reason=("The toolkit (0.12+) sees this as a radical-containing molecule.Need to use a different molecule.")
)
@requires_openeye
def test_generate_omega_conformers_error():
    with pytest.raises(ConformerGenerationError, match="Failed to generate conformers using OMEGA"):
    ...
    with pytest.raises(ConformerGenerationError, match="ELF10 conformer selection failed"):
...
@requires_openeye
def test_generate_conformers(method: str, sampling_mode: str, max_conformers: int | None):
    ...
    ConformerGenerator.generate(
        molecule,
        ConformerSettings(method=method, sampling_mode=sampling_mode, max_conformers=max_conformers),
    )
